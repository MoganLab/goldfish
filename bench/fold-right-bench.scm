...
(define (verify)
  ...
  (let ((test-list-1 (make-test-list 10))
    ...
    ;; 测试双列表 fold-right
    (let ((result-old (fold-right-old +
                        0
                        test-list-1
                        test-list-2
                      ) ;fold-right-old
          ) ;result-old
          (result-new (fold-right-new +
                        0
                        test-list-1
                        test-list-2
                      ) ;fold-right-new
  ...
  (let ((small-list (make-test-list 10)))
    (timing "旧版本:\t\t"
      (lambda ()
        (repeat 5000
          (lambda ()
            (fold-right-old cons '() small-list)
          ) ;lambda
        ) ;repeat
      ) ;lambda
    ) ;timing
    (timing "新版本:\t\t"
      (lambda ()
        (repeat 5000
          (lambda ()
            (fold-right-new cons '() small-list)
          ) ;lambda
        ) ;repeat
      ) ;lambda
    ) ;timing
