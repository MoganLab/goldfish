  ...
  (let ((total-time 0) (sample-result #f))
    (do ((i 0 (+ i 1)))
      ...
      (let-values (((result elapsed-time)
                    (time-thunk (lambda () (func suffix str))
                    ) ;time-thunk
                   ) ;
                  ) ;
        (set! total-time
...
(define (execute-test-case case-name
  ...
  (newline)
  (display "  String: \"")
  (display (if (> (string-length str) 50)
             (string-append "..."
               (substring str
                 (max 0 (- (string-length str) 47))
                 (string-length str)
               ) ;substring
             ) ;string-append
             str
  ...
  (run-test-for-function "Original (char-by-char)"
    string-suffix?-original
    suffix
    str
    num-iterations
  ) ;run-test-for-function
  (run-test-for-function "Optimized (substring)"
    string-suffix?-optimized
    suffix
    str
    num-iterations
  ) ;run-test-for-function
  (run-test-for-function "SRFI-13 current"
    string-suffix?
    suffix
    str
    num-iterations
...
(define (run-all-tests . num-iterations-arg)
  (let ((num-iterations (if (and (pair? num-iterations-arg)
    ...
    (display "Starting performance tests for string-suffix? implementations..."
    ) ;display
    (newline)
    (display "Using ")
    (display num-iterations)
    ...
    (execute-test-case "Short suffix, short string (match)"
      "lo"
      "hello"
      num-iterations
    ) ;execute-test-case
    ...
    (execute-test-case "Short suffix, short string (no match)"
      "hi"
      "hello"
      num-iterations
    ) ;execute-test-case
    ...
    ;; Test 3: Empty suffix
    (execute-test-case "Empty suffix"
      ""
      "hello world"
      num-iterations
    ) ;execute-test-case
    ...
    (let ((long-string (make-mixed-string 1000)))
      (execute-test-case "Long suffix, long string (match)"
    ...
    (let ((long-string (make-mixed-string 1000)))
      (execute-test-case "Long suffix, long string (no match - differs at end)"
        "xyz"
        long-string
        num-iterations
      ) ;execute-test-case
    ...
    (let* ((long-string (make-mixed-string 1000))
           (almost-suffix (string-append "X"
                            (substring long-string 901 1000)
                          ) ;string-append
           ) ;almost-suffix
          ) ;
      (execute-test-case "Long suffix, long string (no match - differs at beginning of suffix)"
        almost-suffix
        long-string
        num-iterations
      ) ;execute-test-case
    ...
    (let ((very-long-string (make-mixed-string 10000)
      ...
      (execute-test-case "Very long suffix, very long string (match)"
    ...
    (execute-test-case "Suffix same length as string (match)"
      "hello"
      "hello"
      num-iterations
    ) ;execute-test-case
    ...
    (execute-test-case "Suffix longer than string"
      "hello world"
      "hello"
      num-iterations
    ) ;execute-test-case
    ...
    (execute-test-case "File extension pattern (match)"
      ".txt"
      "document.txt"
      num-iterations
    ) ;execute-test-case
    ...
    ;; Test 11: URL pattern
    (execute-test-case "URL pattern (match)"
      ".com"
      "www.example.com"
      num-iterations
    ) ;execute-test-case
    ...
    (execute-test-case "Long common suffix (match)"
      "tion"
