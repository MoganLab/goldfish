...
;; base64-decode 性能基准：仅测统一入口 base64-decode（按参数类型分发到 string / bytevector）
...
(do ((i 0 (+ i 1)))
  ((= i 5))
  (base64-decode enc-ascii-medium)
  (base64-decode enc-bv-medium)
) ;do

(display "=== base64-decode 性能基准测试 ===")
(newline)
(newline)

(let ((t (timeit (lambda () (base64-decode enc-ascii-medium)) '() iter)))
  (report "base64-decode string(1KB)" iter t)
) ;let
(let ((t (timeit (lambda () (base64-decode enc-bv-medium)) '() iter)))
  (report "base64-decode bytevector(1KB)" iter t)
) ;let
