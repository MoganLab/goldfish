...
//
// Turns the repetitive "unpack gf::pointer args, call a plain C++ function,
...
//   * f_##fn: gf::function that unpacks args (via s7_traits<T>::in, with
...
//   std::string, gf::int_, int, bool, double, char32_t,
...
//   gf::pointer (pass-through), void (-> (values)).
//
...
// callbacks, reader eval) fall back to a handwritten gf::function.
//
...
// handwritten gf::function stays perfectly acceptable in those cases.
...
inline gf::pointer
gf_error (gf::scheme* sc, const char* kind, const std::string& msg) {
  return s7_error (sc, s7_make_symbol (sc, kind), s7_make_string (sc, msg.c_str ()));
}

// ---------------------------------------------------------------------------
// s7_traits<T>: convert T <-> gf::pointer.
// ---------------------------------------------------------------------------
...
template <> struct s7_traits<std::string> {
  ...
  static bool accepts (gf::pointer p) { return s7_is_string (p); }
  static std::string in (gf::scheme* sc, gf::pointer p) {
    if (!s7_is_string (p))
      gf_error (sc, "type-error", "expected a string argument");
    return std::string (s7_string (p));
  }
  static gf::pointer out (gf::scheme* sc, const std::string& v) {
...
template <> struct s7_traits<char32_t> {
  ...
  static bool accepts (gf::pointer p) { return s7_is_character (p); }
  static char32_t in (gf::scheme* sc, gf::pointer p) {
    if (!s7_is_character (p))
      gf_error (sc, "type-error", "expected a character argument");
    return (char32_t) s7_character (p);
  }
  static gf::pointer out (gf::scheme* sc, char32_t v) {
    return s7_make_character (sc, (uint32_t) v);
  }
};

template <> struct s7_traits<gf::int_> {
  static const char* type_name () { return "integer"; }
  static bool accepts (gf::pointer p) { return s7_is_integer (p); }
  static gf::int_ in (gf::scheme* sc, gf::pointer p) {
    if (!s7_is_integer (p))
      gf_error (sc, "type-error", "expected an integer argument");
    return s7_integer (p);
  }
  static gf::pointer out (gf::scheme* sc, gf::int_ v) { return s7_make_integer (sc, v); }
};

template <> struct s7_traits<int> {
  static const char* type_name () { return "integer"; }
  static bool accepts (gf::pointer p) { return s7_is_integer (p); }
  static int in (gf::scheme* sc, gf::pointer p) {
    return (int) s7_traits<gf::int_>::in (sc, p);
  }
  static gf::pointer out (gf::scheme* sc, int v) { return s7_make_integer (sc, v); }
};

template <> struct s7_traits<bool> {
  static const char* type_name () { return "boolean"; }
  static bool accepts (gf::pointer p) { return s7_is_boolean (p); }
  static bool in (gf::scheme* sc, gf::pointer p) {
    if (!s7_is_boolean (p))
      gf_error (sc, "type-error", "expected a boolean argument");
    return s7_boolean (sc, p);
  }
  static gf::pointer out (gf::scheme* sc, bool v) { return s7_make_boolean (sc, v); }
};

template <> struct s7_traits<double> {
  static const char* type_name () { return "number"; }
  static bool accepts (gf::pointer p) { return s7_is_number (p); }
  static double in (gf::scheme* sc, gf::pointer p) {
    if (!s7_is_number (p))
      gf_error (sc, "type-error", "expected a number argument");
    return s7_number_to_real (sc, p);
  }
  static gf::pointer out (gf::scheme* sc, double v) { return s7_make_real (sc, v); }
};

template <> struct s7_traits<std::vector<std::string>> {
  static const char* type_name () { return "vector-of-strings"; }
  static bool accepts (gf::pointer p) { return s7_is_vector (p); }
  static std::vector<std::string> in (gf::scheme* sc, gf::pointer p) {
    std::vector<std::string> r;
    if (!s7_is_vector (p))
      gf_error (sc, "type-error", "expected a vector argument");
    gf::int_ n= s7_vector_length (p);
    r.reserve (n);
    for (gf::int_ i= 0; i < n; i++)
      r.push_back (std::string (s7_string (s7_vector_ref (sc, p, i))));
    return r;
  }
  static gf::pointer out (gf::scheme* sc, const std::vector<std::string>& v) {
    gf::pointer r= s7_make_vector (sc, (gf::int_) v.size ());
    for (size_t i= 0; i < v.size (); i++)
      s7_vector_set (sc, r, (gf::int_) i, s7_make_string (sc, v[i].c_str ()));
    return r;
  }
};
...
template <> struct s7_traits<std::vector<uint8_t>> {
  ...
  static bool accepts (gf::pointer p) { return s7_is_byte_vector (p); }
  static std::vector<uint8_t> in (gf::scheme* sc, gf::pointer p) {
  ...
  static gf::pointer out (gf::scheme* sc, const std::vector<uint8_t>& v) {
    gf::int_ n= (gf::int_) v.size ();
    gf::pointer r= s7_make_byte_vector (sc, n, 1, NULL);
    if (n > 0)
...
// gf::pointer: pass-through (no conversion).  For functions that need the raw
// s7 object (e.g. to hand it to s7_call later).
template <> struct s7_traits<gf::pointer> {
  static const char* type_name () { return "object"; }
  static gf::pointer in (gf::scheme* sc, gf::pointer p) { return p; }
  static gf::pointer out (gf::scheme* sc, gf::pointer v) { return v; }
};

// void: s7_unspecified (the (values) value).
template <> struct s7_traits<void> {
  static const char* type_name () { return "void"; }
  static gf::pointer out (gf::scheme* sc) { return s7_unspecified (sc); }
};
...
struct s7_traits<std::optional<T>> {
  ...
  static bool accepts (gf::pointer p) { return s7_traits<T>::accepts (p); }
  static std::optional<T> in (gf::scheme* sc, gf::pointer p) {
    if (s7_is_boolean (p) && !s7_boolean (sc, p)) return std::nullopt;
    return std::make_optional (s7_traits<T>::in (sc, p));
  }
  static gf::pointer out (gf::scheme* sc, const std::optional<T>& v) {
...
template <> struct s7_traits<gf_strlist> {
  ...
  static gf::pointer out (gf::scheme* sc, const gf_strlist& v) {
    gf::pointer head= s7_cons (sc, s7_nil (sc), s7_nil (sc));
    s7_gc_protect_via_stack (sc, head);
    gf::pointer tail= head;
...
struct s7_traits<std::variant<Ts...>> {
  ...
  static std::variant<Ts...> in (gf::scheme* sc, gf::pointer p) {
  ...
  template <typename T>
  static void try_alternative (gf::scheme* sc, gf::pointer p, std::variant<Ts...>& result, bool& ok) {
    if (ok) return;
...
struct gf_arg_reader {
  static std::decay_t<T> read (gf::scheme* sc, gf::pointer p) {
...
std::decay_t<T> gf_read_arg (gf::scheme* sc, gf::pointer args) {
  gf::pointer p= args;
...
Tuple gf_read_all (gf::scheme* sc, gf::pointer args, std::index_sequence<I...>) {
  return Tuple (gf_read_arg<I, std::tuple_element_t<I, Tuple>> (sc, args)...);
}

template <typename Fn>
auto gf_read_args (gf::scheme* sc, gf::pointer args) {
...
gf::pointer gf_call (gf::scheme* sc, Fn fn, Tuple& t, std::index_sequence<I...>, std::false_type /*not void*/) {
...
template <typename Fn, typename Tuple, size_t... I>
gf::pointer gf_call (gf::scheme* sc, Fn fn, Tuple& t, std::index_sequence<I...>, std::true_type /*void*/) {
  try {
    fn (std::get<I> (t)...);
...
gf::pointer gf_dispatch (gf::scheme* sc, Fn fn, Tuple& t) {
  using traits = gf_fn_traits<Fn>;
...
//
// Expands to:
//   static gf::pointer f_##fn (gf::scheme* sc, gf::pointer args);
//   static void glue_##fn (gf::scheme* sc);
...
#define GF_GLUE(Scheme_Name, Desc, fn)                                             \
  ...
  static void glue_##fn (gf::scheme* sc) {                                          \
    s7_define_typed_function (sc, Scheme_Name, f_##fn,                             \
                              (gf::int_) goldfish::gf_fn_traits<decltype (&fn)>::arity, \
                              0, false, Desc, NULL);                               \
  }
