...
static void push_frame (s7_scheme* sc, int code_idx, const std::vector<s7_pointer>& args, s7_pointer captured, std::vector<s7_pointer>* reuse) {
  VMCodeInfo& ci = g_program->codes[code_idx];
  VMFrame f;
  f.pc = 0;
  f.code = &ci.instrs;
...
static s7_pointer call_function (s7_scheme* sc, s7_pointer f, const std::vector<s7_pointer>& args, std::vector<s7_pointer>* reuse) {
  if (s7_gf_is_closure (f)) {
    ...
    if (s7_is_pair (body) && s7_is_pair (s7_car (body)) &&
      ...
      push_frame (sc, vc->code_idx, args, vc->captured, reuse);
      return nullptr;
    }
  }
  ...
  while (g_frames.size () > target_depth) {
    ...
    switch (in.op) {
      case Op::Const:
        ...
        break;
      case Op::Global:
        push (s7_gf_global_value (sc, in.a));
        break;
      case Op::Ref:
...
static s7_pointer vm_enter (s7_scheme* sc, s7_pointer args) {
  ...
  push_frame (sc, vc->code_idx, arg_list, vc->captured, nullptr);
...
void glue_vm (s7_scheme* sc) {
  ...
  s7_define_function (sc, "vm-load", vm_load, 1, 0, false, "(vm-load program)");
