...
- **宏层前置工程——syntax-case 剩余缺口**（2026-08 排查）：
  1. pattern 层 `dotted+ellipsis`（`(a ... . r)`）已修：`syntax-runtime.scm` 以 `pair-or-null?`/`dotted-length` 替代 `s7` `list?`/`length` 的 host-ism（`length` 对 dotted 返回 `-len`，`list?` 为 `pair-or-null`）；`s7` 本体该行为为有意设计（`H_length`/`H_is_list`），`goldfish` 在 `(scheme base)` 以 `proper-list?` 覆盖为 `R7RS`（`list?`/`length` 对 improper 报错，见 `goldfish/scheme/base.scm`），宿主保留 `O(1)` 语义供展开器内部复用。
  2. template 层：`generate-temporaries` 多 temp 经 with-syntax + ellipsis splice 插入 lambda formals 后失 `identifier` 性质（"lambda: expected identifier"）；单 temp 路径正常。仍阻塞 `let-values/define-values` 等 `gensym` 类宏的 `syntax-case` 化，须先在 `Guile` 验证（`srfi-11` rest-lambda+解构为权威），再对齐 `goldfish`。
