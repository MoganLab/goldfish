...
- **冻结基线**：`tools/freeze-substrate.sh` 生成、`tools/substrate-baseline.txt` 存档——A `gf::` 转发 120（`src/gf_forwards.def`，含 `is_keyword`、`c_type_set_ref`、`make_complex`、`byte_vector_set`、`is_hash_table`）、B `GF_GLUE` 声明式原语 37、C Scheme 可见 C++ 原语 120（117 个 `g_*` + `iota`/`read`/`version`，含 5 个 gf0 引擎入口＋stale fence 的 `g_gf0-s7call-token`）、D `core-language` 14。lint 逐行 diff，增长须同 commit 更新基线并说明理由。
...
- **文件**：`goldfish/compiler/*.scm`（`syntax-ir.scm` 现为对 `expander/tree-il` 的薄包装；`$` 模式别名在 `compiler/patterns.scm`）、`goldfish/compiler.scm`。
...
- 参考求值器 `src/gf0_eval.cpp`（lowered sexp 直 walk，`g_gf0-eval` 为引擎入口，`gf eval-gf0` 为调试驱动；求值合同见 `CORE-SEMANTICS.md`）——先在 s7 求值平替上站稳，再下沉 VM。
...
- **核心语言**（tree-il 的语法面）：`quote define lambda if begin let let* letrec letrec* set!` 十个 special forms，加 `values call-with-values`（多值派生）与 `module-ref module-set`（跨库引用）——权威定义在 `goldfish/core/ir.scm` 的 `core-language` 表（含到 IR 节点的映射与文法）；其余一切形式按定义皆为调用。管线变更时的执法走 `core-form?`/`core-node-of`。
- 其余为底层语言内建（算术/string/vector/hash-table/port…），按 R7RS-small + 必要扩展对齐，不逐一枚举。

### T2 平台能力（非语言；来自 OS/C 标准库，共 116 个 `g_*`，另 `iota`/`read`/`version` 三个非 g_ 名）

- GF_GLUE 声明式原语 37（os 17 / path 11 / hashlib 6 / base64 2 / string-split 1）、njson 32、sys/time/process-context 20（含 4 时钟常量）、http 8、reader 7、char 5 + string->utf8、gf0 引擎入口 5。完整名单见 `tools/substrate-baseline.txt` 表 C。
