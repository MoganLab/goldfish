...
- **s7 的 values/unspecified 折叠语义（上游分歧，未收敛）**：s7 中 `(values x)` 当 x 为 unspecified 时折叠为**零值序列**；thunk 返回 unspecified 时 call-with-values 同样视为空序列。R7RS/Guile 均为单值。影响面：一切以 `(if #f #f)` 类未指定值进入多值协议的移植代码（SRFI-165 已绕过：with! 改产 `'unspecified` 占位）。候选后续：宿主层提供不折叠的多值原语，或与 s7 协商。
