...
- **冻结基线**：`tools/freeze-substrate.sh` 生成、`tools/substrate-baseline.txt` 存档——A `gf::` 转发 115（`src/gf_forwards.def`）、B `GF_GLUE` 声明式原语 37、C Scheme 可见 C++ 原语 119（116 个 `g_*` + `iota`/`read`/`version`，含 5 个 gf0 引擎入口）、D `core-language` 14。lint 逐行 diff，增长须同 commit 更新基线并说明理由。
