  ...
  (begin
    ...
    (define (display-usage)
      (let ((port (current-output-port)))
        (display "Usage:" port)
        (newline port)
        (display "  gf source ORG/LIB" port)
        (newline port)
        (newline port)
        (display "Examples:" port)
        (newline port)
        (display "  gf source liii/string        # 显示 (liii string) 库的源代码"
          port
        ) ;display
        (newline port)
        (display "  gf source scheme/base        # 显示 (scheme base) 库的源代码"
          port
        ) ;display
        (newline port)
      ) ;let
