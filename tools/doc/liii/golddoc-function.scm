...
(define-library (liii golddoc-function)
  ...
  (export exported-name->test-stem library-documented-functions function-doc-path)
  (begin

    (define golddoc-section-headings
      '("语法" "参数" "返回值" "描述" "说明" "注意" "错误处理"
        "示例")
    ) ;define
        ...
        (and (> value-length 1)
          (let loop
            ...
            (if (>= index value-length)
              #t
              (and (char=? (string-ref value index) #\-) (loop (+ index 1)))
            ) ;if
          ) ;let
        ) ;and
      ) ;let
      ...
      (let loop
        ...
        (and (not (null? lines))
          (let ((body (comment-line->body (car lines)))
            ...
            (or (and candidate (string=? candidate exported-name))
              (and body
    ...
    (define (function-doc-path library-query exported-name)
      (let* ((parts (parse-library-query library-query))
        ...
        (cond ((and candidate (path-file? candidate)) candidate)
              (library-dir (find-function-doc-by-scan library-dir exported-name))
              (else #f)
        ) ;cond
      ) ;let*
    ) ;define
