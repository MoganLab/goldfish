...
(when (not (os-windows?))
  (let* ((base-root (path-join (path-temp-dir)
                      (string-append "golddoc-library-without-docs-" (number->string (getpid)))
                    ) ;path-join
         ) ;base-root
    ...
    (dynamic-wind (lambda () (path-unlink output-path #t))
      (lambda ()
        ...
        (let ((output (path-read-text output-path)))
          (check-true (string-contains? output "Library (liii demo) exists."))
          (check-true (string-contains? output "No documentation and test cases available.")
          ) ;check-true
          (check-true (string-contains? output "Try one of these commands:"))
          (check-true (string-contains? output (string-append command-name " source liii/demo"))
          ) ;check-true
        ) ;let
      ) ;lambda
      (lambda ()
