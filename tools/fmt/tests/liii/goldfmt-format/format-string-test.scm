...
;; 2. 多行 keyword 调用（首参为 keyword）：首行不留参数，各 key-value 成对同行换行且 value 列对齐
...
;; 3. 带位置参数的 keyword 调用：位置参数在首行，后续 keyword 对换行且 value 列对齐
(check (format-string "(model-profile mp :model \"kimi-k3\" :base-url \"/api/v1/ai/siliconflow/chat\" :max-tokens 16384)")
  =>
  "(model-profile mp\n  :model      \"kimi-k3\"\n  :base-url   \"/api/v1/ai/siliconflow/chat\"\n  :max-tokens 16384\n) ;model-profile\n"
) ;check

;; 4. run-values 等相邻 keyword 调用，value 统一列对齐
(check (format-string "(run-values command :cwd cwd :env env :input input :timeout timeout :stdout stdout :stderr stderr :stdin stdin)")
  =>
  "(run-values command\n  :cwd     cwd\n  :env     env\n  :input   input\n  :timeout timeout\n  :stdout  stdout\n  :stderr  stderr\n  :stdin   stdin\n) ;run-values\n"
) ;check

;; 5. value 为多行表达式（如多行 if）：:model 与 if 换行且同列缩进；短 if 仍与 key 同行
...
;; 6. value 超长（> 80 列）：value 另起一行且与 key 同列缩进
...
;; 7. define-record-type 等定义形式中的类型名为 keyword 时不误当成 keyword 参数
