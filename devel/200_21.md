...
## 2026/02/24 将 infinite?/finite? 相关实现（infinite?）从 s7.c 迁移到 s7_scheme_inexact.c
### What
将 infinite? 函数从 s7.c 迁移到 s7_scheme_inexact.c（finite? 依赖 infinite?），包括：
1. 迁移 s7_is_infinite 函数实现，移除 WITH_GMP 条件编译代码
2. 迁移 g_is_infinite 函数实现
3. 在 s7_scheme_inexact.h 中添加函数声明
4. 在 s7.c 中移除迁移的代码
5. 在 s7.c 中更新函数注册，将 defun 宏调用改为直接调用 s7_define_typed_function
6. 保持 scheme inexact 库中的 finite? 行为不变（仍由 infinite? 与 nan? 组合判断）

## 2026/02/24 将 log 从 s7.c 迁移到 s7_scheme_inexact.c
### What
将 log 函数从 s7.c 迁移到 s7_scheme_inexact.c，包括：
1. 迁移 g_log 函数实现，移除 WITH_GMP 条件编译代码
2. 保留 `(log x)` 与 `(log x base)` 的行为分支（含 base=2 快路径、复数分支、异常分支）
3. 在 s7_scheme_inexact.h 中添加函数声明
4. 在 s7.c 中移除迁移的代码
5. 在 s7.c 中更新函数注册，将 defun 宏调用改为直接调用 s7_define_typed_function
6. 保持 `g_int_log2` 和 `log_chooser` 的优化器路径不变

## 2026/02/24 将 exp 从 s7.c 迁移到 s7_scheme_inexact.c
### What
将 exp 函数从 s7.c 迁移到 s7_scheme_inexact.c，包括：
1. 迁移 exp_p_p 函数实现，移除 WITH_GMP 条件编译代码
2. 迁移 g_exp 函数实现
3. 迁移 exp_p_d 函数实现
4. 迁移 exp_d_d 函数实现
5. 在 s7_scheme_inexact.h 中添加函数声明
6. 在 s7.c 中移除迁移的代码
7. 保持 s7.c 中函数注册和优化器调用点不变
