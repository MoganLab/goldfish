...
## 2026/2/26 新增`njson-set!`，`njson-push!`，`njson-drop!`并优化`json-keys`（懒重建缓存）
...
3. keys 性能优化：
   ...
   在 `set!/push!/drop!` 原地更新成功后，若缓存存在则仅失效；下一次 `njson-keys` 再懒重建。
4. 测试补充：
...
3. 生命周期管理：
   ...
   - 可变更新成功后调用 `njson_invalidate_keys_cache_if_present` 仅失效缓存，读路径再重建。
4. keys 构造微优化：
...
### 基准结果（本次实测，节选）
- 读取(json-ref): `liii/nlohmann = 1112.49x`
- 原地修改对比(liii-set vs njson-set!): `294.53x`
- 原地插入对比(liii-push vs njson-push!+drop!): `82.34x`
- 原地删除对比(liii-drop vs njson-set!+drop!): `155.13x`
- 获取键(json-keys): `liii/nlohmann = 392.66x`
