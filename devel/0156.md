...
### What
1. 修复 `src/goldfish.hpp` 中的 `iota_list`：循环原先在每次消费 `val` 后均无条件执行 `val -= step`，导致最后一轮迭代结束后多算一次递减。对于合法且极值贴边的调用（如 `(iota 2 9223372036854775807 -1)`），多余的一次递减会导致 `INT64_MAX + 1`，触发有符号整数溢出 UB（在 UBSan 下 abort）。
2. 将 `val -= step` 改为在判断需要继续下一轮迭代时才执行，消除末次迭代多余的加减运算。
3. 在 `tests/liii/list/iota-test.scm` 中增加对 `INT64_MAX` 和 `INT64_MIN` 贴边但合法的边界测试用例。
