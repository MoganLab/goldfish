...
### 5.1 What
在 XPack Build 工作流中新增 `package-macos` job，为 macOS (arm64 与 x86_64) 产出预编译的 `.tar.gz` 包并发布到 release。
...
3. 在 `package.yml` 中新增 `package-macos` job，通过 matrix 同时在 `macos-14`(arm64) 和 `macos-13`(x86_64) 上构建、打包并发布。
...
- **架构覆盖**：用 `matrix` 同时跑 `macos-14`(arm64) 与 `macos-13`(x86_64)，兼顾 Apple Silicon 与 Intel Mac。
- **产物校验**：在 job 中加入 `Verify package arch` 步骤，断言产物文件名里包含 matrix 期望的 arch，防止 runner 的实际架构与 matrix 配置不一致时悄悄产出错误命名的包。
