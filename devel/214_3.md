...
## 2026/2/25 新增 njson 自动释放宏

### What
1. `goldfish/liii/njson.scm`：
   在导出列表中加入 `njson-with-json` 与 `njson-with-value`。
2. 宏实现重构：
   新增 `njson%%single-binding?`、`njson%%binding-list?`、`njson%%normalize-bindings`、
   `njson%%json-binding->value-binding`、`njson%%json-bindings->value-bindings`、
   `njson%%expand-with-value-bindings`，统一绑定解析和代码展开路径。
3. 宏卫生与语法校验：
   `njson-with-value` 在展开阶段使用 `gensym("njson-released?")` 生成临时变量；
   绑定列表要求非空，`(njson-with-json ())` 与 `(njson-with-value ())` 抛 `type-error`。
4. `tests/goldfish/liii/njson-test.scm`：
   由“直接句柄 + 手动 free”为主的写法，调整为“按 API 分段 + 作用域宏”为主；
   增加块注释文档，测试组织方式参考 `json-test.scm`；
   将大量嵌套单绑定改为多绑定写法，覆盖多 JSON 同时处理场景。
5. 测试补充：
   新增 `njson-with-json` / `njson-with-value` 空绑定报错用例；
...
2. `njson-with-json` 统一转换为 `njson-with-value` 绑定格式：
   `(var json-string)` -> `(var (njson-string->json json-string))`。
...
### 兼容性说明
1. 保持兼容：`(njson-with-json (j s) ...)` 与 `(njson-with-json ((j s)) ...)` 仍可用。
2. 行为变更：空绑定列表从“直接执行 body”调整为抛 `type-error`。
3. `njson-test.scm` 的组织结构与注释体系有较大变动，但接口语义不变。

## 验证结果
本次文档编写前已执行：
1. ✅ `bin/goldfish tests/goldfish/liii/njson-test.scm` 通过。
2. ✅ 输出：`; *** checks *** : 91 correct, 0 failed.`
