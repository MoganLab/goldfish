...
bin/gf tests/liii/json/json-string-escape-test.scm
bin/gf bench/json-ref-set-perf.scm
```

## 2026-08-19 json-string-escape 迁移到 C++，删除 (guenchi json) 模块

### What
1. `tests/liii/json/json-string-escape-test.scm` 补全测试用例：多字节 UTF-8 字符（中文、é、emoji、混合转义、500 个 `#\中` 长字符串）与裸控制字符转义（`#\x1`、`#\x1f`、`#\null` → `\uXXXX`）
2. `src/liii_json.cpp` 新增 `g_json_string_escape`，复用 `json->string` 的 `json_write_escaped_string`（含快路径）：`\" \\ \/ \b \f \n \r \t`，其余控制字符（< 0x20）转义为 `\uXXXX`，多字节 UTF-8 原样输出；非字符串参数抛 wrong-type-arg
3. `(liii json)` 的 `json-string-escape` 直接绑定 `g_json_string_escape`，移除对 `(guenchi json)` 的 import
4. 删除 `goldfish/guenchi/` 目录（仅剩 `json.scm` 一个文件），并清理 `xmake.lua` 中对应的 installfiles 规则

### Why
`json-string-escape` 是 `(guenchi json)` 最后一个接口；其 Scheme 实现逐字符走 `bytevector-advance-utf8` + `write-char`/`display`，远慢于 C++ 的成段拷贝快路径。迁移后 `(guenchi json)` 模块不再有任何使用者，整体删除。

### How
- 直接复用 `json_write_escaped_string`，与 `json->string` 的字符串转义行为完全一致
- 语义变化（改进）：旧 Scheme 实现对 < 0x20 的其他控制字符原样输出（不可再解析），迁移后改为 `\uXXXX` 转义，保证输出是合法 JSON；已有测试锁定新行为
