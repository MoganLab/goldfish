...
;; 注册宏
;; 注意：s7 *#readers* 调用 hook 时，str 参数包含 '#' 之后的已读内容。
;; 对于 #'hello，str = "'hello"（s7 已把 'hello 整体读完）。
;; 对于 #'(foo bar)，str = "'"（s7 看到 '( 就停在 ( 前面）。
;; 因此对于原子符号必须从 str 提取，不能再从 port 读取。
(set! *#readers*
  (cons
    (cons #\' (lambda (str)
                (if (> (string-length str) 1)
                    ;; str = "'hello" => 原子符号，从 str 直接提取
                    (list 'syntax (string->symbol (substring str 1)))
                    ;; str = "'" => 后面是复合结构如 (foo)，从 port 读取
