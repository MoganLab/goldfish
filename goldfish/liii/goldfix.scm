(define-library (liii goldfix)
  ...
  (begin
    ...
    ;; case* 的字面量模式可能形如 (((if ...)) ...)。
    ;; 这里的 if/lambda 等是数据，不是实际 env。
    ;; 如果前面阶段误把匿名右标记拆成独立的 ") ;" 行，则在最终输出前并回上一行。
    (define (merge-case-pattern-keyword-right-tags lines)
      (let loop ((remaining lines)
        ...
        (if (null? remaining)
          ...
          (if (and (pair? result)
                   ...
                   (string-starts? (string-trim (cadr remaining)) "(else")
                   (line-ends-with-rparen? (car result))
                   (case-pattern-keyword-line? (cadr result)))
            (loop (cdr remaining)
    ...
    (define (run-goldfix-lines lines)
      (let* ((prepared-lines (repair-define-header-lines lines))
             ...
             (best-lines (or (first-healthy-lines
                             ...
                             prepared-lines))
             (canonical-lines (merge-case-pattern-keyword-right-tags
