...
(define (utf8-codepoint-at bv pos) (let ((b0 (bytevector-u8-ref bv pos))) (cond ((<= b0 127) b0) ((<= b0 223) (logior (ash (logand b0 31) 6) (logand (bytevector-u8-ref bv (+ pos 1)) 63))) ((<= b0 239) (logior (ash (logand b0 15) 12) (ash (logand (bytevector-u8-ref bv (+ pos 1)) 63) 6) (logand (bytevector-u8-ref bv (+ pos 2)) 63))) (else (logior (ash (logand b0 7) 18) (ash (logand (bytevector-u8-ref bv (+ pos 1)) 63) 12) (ash (logand (bytevector-u8-ref bv (+ pos 2)) 63) 6) (logand (bytevector-u8-ref bv (+ pos 3)) 63))))))
(define (utf8-string->chars str) (let ((bv (string->byte-vector str)) (len (string-length str))) (let loop ((pos 0) (acc '())) (if (>= pos len) (reverse acc) (let ((next (bytevector-advance-utf8 bv pos len))) (if (= next pos) (error (quote value-error) "invalid UTF-8 sequence at index: " pos) (loop next (cons (integer->char (utf8-codepoint-at bv pos)) acc))))))))
(define (codepoint->utf8-bytes cp) (cond ((<= cp 127) (list cp)) ((<= cp 2047) (list (logior 192 (ash cp -6)) (logior 128 (logand cp 63)))) ((<= cp 65535) (list (logior 224 (ash cp -12)) (logior 128 (logand (ash cp -6) 63)) (logior 128 (logand cp 63)))) (else (list (logior 240 (ash cp -18)) (logior 128 (logand (ash cp -12) 63)) (logior 128 (logand (ash cp -6) 63)) (logior 128 (logand cp 63))))))
(define (utf8-chars->string ls) (let* ((bls (apply append (map codepoint->utf8-bytes (map char->integer ls)))) (bv (make-bytevector (length bls)))) (let loop ((i 0) (b bls)) (if (null? b) (utf8->string bv) (begin (bytevector-u8-set! bv i (car b)) (loop (+ i 1) (cdr b)))))))
(define (string-map p . args) (if (not (procedure? p)) (error (quote wrong-type-arg) "string-map: procedure expected" p) (if (null? args) "" (utf8-chars->string (apply map p (map utf8-string->chars args))))))
(define (string-for-each p . args) (if (not (procedure? p)) (error (quote wrong-type-arg) "string-for-each: procedure expected" p) (apply for-each p (map utf8-string->chars args))))
...
(define (write-bytevector bv . rest)
  ;; R7RS 参数顺序：(write-bytevector bv [port [start [end]]])
  ;; 同时兼容旧的 (write-bytevector bv start [end]) 顺序
...
(define (read-bytevector! bv . rest)
  ;; R7RS 参数顺序：(read-bytevector! bv [port [start [end]]])
  ;; 同时兼容旧的 (read-bytevector! bv start [end]) 顺序
