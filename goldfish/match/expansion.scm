;;; match/expansion.scm
;;; Expansion-time machinery for the (goldfish match) macros (v5):
;;; own value definitions are phase-0 only, so the helpers the match
;;; transformers call live in this library, imported plainly by
;;; (goldfish match) -- visible at every phase via the substrate rule.
(define-library (goldfish match expansion)
  (import (goldfish) (scheme base))
  (export
    ell? pattern-transformer seq-range last-elem drop-right
    expand-pattern has-ellipsis? extended-ell? flatten-sub-ellipsis
    parse-seq-subpats expand-seq-subpats expand-seq-form
    expand-cons*-plain expand-cons*-ellipsis expand-list-ellipsis
    expand-vector-ellipsis expand-quasiquote-form expand-quasiquote
    expand-seq-list gen* gen-and-pred* gen-list* gen-vector*
    gen-vector-elems* gen-or* gen-not* gen-proj* gen-instructions
    gen-seq* gen-unordered* rename-body collect-vars
    compile-pats-gen* compile-one-clause compile-group)
  (begin
