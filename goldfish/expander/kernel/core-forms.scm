...
(define (stx-cadr stx) (stx-propagate-wrap stx (cadr (syntax-form stx))))
...
(define (core-lambda stx ctx)
  (let* ((form (syntax-form stx))
         (params-stx (stx-propagate-wrap stx (cadr form)))
         (body-stxs (map (lambda (s) (stx-propagate-wrap stx s)) (cddr form)))
    ...
    ((and (syntax? params-stx) (pair? (syntax-form params-stx)))
     (let loop ((p (syntax-form params-stx)) (fixed '()))
       (cond
         ((pair? p)
          (loop (cdr p)
                (cons (stx-propagate-wrap params-stx (car p)) fixed)))
         ((null? p) (values (reverse fixed) #f))
         ((identifier? p) (values (reverse fixed)
                                  (stx-propagate-wrap params-stx p)))
...
(define (core-if stx ctx)
  (let ((form (syntax-form stx)))
    (cond
      ((= 3 (length form))
       (let*-values (((c-sexp c1) (expand-expr (stx-propagate-wrap stx (cadr form)) ctx))
                     ((t-sexp c2) (expand-expr (stx-propagate-wrap stx (caddr form)) c1)))
      ...
      ((= 4 (length form))
       (let*-values (((c-sexp c1) (expand-expr (cadr form) ctx))
                     ...
                     ((e-sexp c3) (expand-expr (stx-propagate-wrap stx (cadddr form)) c2)))
...
(define (stopped-form? stx ctx)
  (and (syntax? stx)
       ...
       (let*-values (((name binding)
                      (resolve-identifier (stx-propagate-wrap stx (car (syntax-form stx))) ctx)))
         (tstop-binding? binding))))

(define (core-begin stx ctx)
  (let* ((form (syntax-form stx))
         (body (map (lambda (s) (stx-propagate-wrap stx s)) (cdr form))))
...
(define (core-set! stx ctx)
  (let ((form (syntax-form stx)))
    ...
    (let ((var-stx (stx-propagate-wrap stx (cadr form)))
          (val-stx (stx-propagate-wrap stx (caddr form))))
      (if (pair? (syntax-form var-stx))
        (let* ((target-form (syntax-form var-stx))
               (proc-stx (stx-propagate-wrap var-stx (car target-form)))
               (arg-stxs (map (lambda (s) (stx-propagate-wrap var-stx s))
                              (cdr target-form))))
...
(define (expand-letrec-form stx ctx form-name)
  (let* ((form (syntax-form stx))
         (binding-stxs (syntax-form (stx-propagate-wrap stx (cadr form))))
         (body-stxs (map (lambda (s) (stx-propagate-wrap stx s)) (cddr form)))
         (ph (context-phase ctx)))
    (let*-values (((scp ctx1) (context-alloc-scope ctx)))
  ...
  (let loop ((bs binding-stxs) (c ctx) (names '()))
    (if (null? bs)
        ...
        (let* ((bs-stx (car bs))
               ...
               (id (stx-add-scope (stx-propagate-wrap bs-stx (car b)) scp ph)))
...
(define (expand-letrec-inits binding-stxs ctx names scp ph)
  (let loop ((bs binding-stxs) (c (context-reset-use-scopes ctx)) (inits '()))
    (if (null? bs)
        ...
        (let* ((bs-stx (car bs))
               (b (syntax-form bs-stx))
               (init-stx (stx-add-scope (stx-propagate-wrap bs-stx (cadr b)) scp ph)))
...
(define (expand-syntax-bindings/rec binding-stxs ctx scp ph)
  (if (null? binding-stxs)
      ...
      (let* ((bs-stx (car binding-stxs))
             (b (syntax-form bs-stx))
             (id (stx-add-scope (stx-propagate-wrap bs-stx (car b)) scp ph))
             (transformer-stx (stx-add-scope (stx-propagate-wrap bs-stx (cadr b)) scp ph)))
...
(define (expand-syntax-bindings binding-stxs ctx scp ph)
  (if (null? binding-stxs)
      ...
      (let* ((bs-stx (car binding-stxs))
             (b (syntax-form bs-stx))
             (id (stx-add-scope (stx-propagate-wrap bs-stx (car b)) scp ph))
             (transformer-stx (stx-propagate-wrap bs-stx (cadr b))))
