...
;;; Pattern matching
;;;
;;; pattern-match-list : (list datum) (list syntax) syntax (list symbol) (list pair) -> bindings or #f

;;; Literals are an alist of (symbol . literal-id-stx): matching uses
;;; free-identifier=? when an expansion context is available, so a
;;; shadowed literal (e.g. locally bound `else`) no longer matches.
...
(define (literal-matches? pattern input literals)
  (if (syntax? input)
      (if (symbol? (syntax-form input))
          (letrec* ((entry (assq pattern literals)))
            (if (and entry (syntax? (cdr entry)) (current-expand-context))
                (free-identifier=? input (cdr entry))
                (eq? (syntax-form input) pattern)))
          #f)
      #f))
...
(define (pattern-match* pattern input literals bindings)
  (if (pattern-variable? pattern literals)
      ...
      (if (eq? pattern '_)
          ;; `_` is normally a wildcard, but when listed among the literals it
          ;; matches only the identifier `_` itself (the reference behaviour
          ;; match.scm relies on to tell a wildcard pattern from a binder).
          (if (assq '_ literals)
          ...
          (if (symbol? pattern)
              ...
              (if (vector? pattern)
                  ...
                  (if (not (pair? pattern))
                      (if (and (syntax? input)
                               (equal? (syntax-form input) pattern))
                          bindings
                          (if (equal? input pattern) bindings #f))
                      (if (not (syntax? input))
                          #f
...
(define (pattern-match-tail pat-tail input-form input-stx literals bindings)
  (if (pattern-variable? pat-tail literals)
      (cons (cons pat-tail (datum->syntax input-stx input-form)) bindings)
      (if (eq? pat-tail '_)
          bindings
...
(define (syntax-case-dispatch input literals-stx clauses)
  (letrec* ((literals (map (lambda (l) (cons (syntax->datum l) l))
                           (if (syntax? literals-stx) (syntax-form literals-stx) '())))
            (loop (lambda (cls)
                    (if (null? cls)
