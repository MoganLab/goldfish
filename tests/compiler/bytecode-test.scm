...
;; L2 字节码编译前端单元测试（record IR 版）。
;;
;; to-bytecode 把 IR record 树编译成可序列化指令序列（纯数据），
;; 是字节码 VM 后端的编译器前端。约定：
;;   尾位置应用 -> (tail-call n)；非尾 -> (call n)；变量有帧槽 ->
;;   (local i)，否则 (ref name)（闭包捕获）。
...
(check (defs->bytecode '((define (add x) (+ x 1))))
       => '(program
            (code-table
             (code 1 ((ref +) (local 0) (const 1) (tail-call 2))))
            (top (closure 0) (store-global add))))

;; ===== 2. if 分支与标签 =====
(check (defs->bytecode '((define (f x) (if (> x 0) 1 2))))
       => '(program
            (code-table
             (code 1 ((ref >) (local 0) (const 0) (call 2)
                       (if-else 0)
                       (const 1) (return)
                       (label 0)
                       (const 2) (return))))
            (top (closure 0) (store-global f))))

;; ===== 3. let 槽分配 =====
(check (defs->bytecode '((define (f x) (let ((a (+ x 1))) (* a 2)))))
       => '(program
            (code-table
             (code 2 ((ref +) (local 0) (const 1) (call 2) (set-local 1)
                       (ref *) (local 1) (const 2) (tail-call 2))))
            (top (closure 0) (store-global f))))
       ...
       => '(program
            (code-table
             (code 1 ((ref =) (local 0) (const 0) (call 2)
                       ...
                       (const 0) (return)
                       (label 0)
                       (ref loop) (ref -) (local 0) (const 1) (call 2)
                       (tail-call 1))))
            (top (closure 0) (store-global loop))))

;; ===== 5. begin 非尾表达式弹出 =====
(check (defs->bytecode '((define (f x) (begin (g x) (h x)))))
       => '(program
            (code-table
             (code 1 ((ref g) (local 0) (call 1) (pop)
                       (ref h) (local 0) (tail-call 1))))
            (top (closure 0) (store-global f))))
...
(check (defs->bytecode '((define (f) (call-with-values
       ...
       => '(program
            (code-table
             (code 2 ((ref +) (local 0) (local 1) (tail-call 2)))
             (code 0 ((const 1) (const 2) (closure 0) (tail-call 2))))
            (top (closure 1) (store-global f))))
...
(check (defs->bytecode '((define (f g) (call-with-values g (lambda (a b) (+ a b))))))
       => '(program
            (code-table
             (code 2 ((ref +) (local 0) (local 1) (tail-call 2)))
             (code 1 ((local 0) (closure 0) (call-with-values) (return))))
            (top (closure 1) (store-global f))))
...
(check (defs->bytecode '((define v (list 1 2))))
       => '(program
            (code-table)
            (top (ref list) (const 1) (const 2) (call 2) (store-global v))))

;; ===== 9. 顶层表达式 =====
(check (defs->bytecode '((display "hi")))
       => '(program
            (code-table)
            (top (ref display) (const "hi") (call 1))))
...
;; ===== 11. 嵌套 lambda：外层变量按名字引用（闭包捕获）=====
(check (defs->bytecode '((define (f x) (lambda () x))))
       => '(program
            (code-table
             (code 0 ((ref x) (return)))
             (code 1 ((closure 0) (return))))
            (top (closure 1) (store-global f))))
...
(check (defs->bytecode '((define (f x) (define (g) x) (g))))
       => '(program
            (code-table
             (code 0 ((ref x) (return)))
             (code 2 ((closure 0) (set-local 1) (local 1) (tail-call 0))))
            (top (closure 1) (store-global f))))
...
(check (defs->bytecode '((define (f) (begin (begin) (f)))))
       => '(program
            (code-table
             (code 0 ((const #f) (pop) (ref f) (tail-call 0))))
            (top (closure 0) (store-global f))))
...
(check (defs->bytecode '((define (f x) (if x (f)))))
       => '(program
            (code-table
             (code 1 ((local 0) (if-else 0)
                       (ref f) (tail-call 0)
                       (label 0)
                       (const #f) (return))))
            (top (closure 0) (store-global f))))

;; dotted formals：rest 参数收单个槽
(check (defs->bytecode '((define (f . rest) rest)))
       => '(program
            (code-table
             (code 1 ((local 0) (return))))
            (top (closure 0) (store-global f))))
...
(check (defs->bytecode '((define (f x) (set! x 1))
       ...
       => '(program
            (code-table
             (code 1 ((const 1) (set-local 0) (return))))
            (top (closure 0) (store-global f)
