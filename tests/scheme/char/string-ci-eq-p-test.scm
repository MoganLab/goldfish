...
;; 多参数测试
(check (string-ci=? "hello" "HELLO" "Hello") => #t)
(check (string-ci=? "HELLO" "hello" "hello") => #t)
(check (string-ci=? "abc" "ABC" "abc") => #t)
(check (string-ci=? "abc" "def" "ABC") => #f)
(check (string-ci=? "ABC" "DEF" "abc") => #f)
;; 空字符串测试
(check (string-ci=? "" "") => #t)
(check (string-ci=? "" "a") => #f)
...
(check (string-ci=? "a1b2" "A1B2") => #t)
;; 特殊字符测试
(check (string-ci=? "hello world" "HELLO WORLD") => #t)
(check (string-ci=? "hello!" "HELLO!") => #t)
(check (string-ci=? "hello@world" "HELLO@WORLD") => #t)
;; 错误处理测试
