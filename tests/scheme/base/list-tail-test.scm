...
(check (list-tail '("first" "second" "third" "fourth") 3)
  =>
  '("fourth")
) ;check
;; 符号列表测试
(check (list-tail '(define lambda if cond else) 2)
  =>
  '(if cond else)
) ;check
...
(check (list-tail (append '(a b c) (make-list 997 'x)) 3)
  =>
  (make-list 997 'x)
) ;check
;; 边界测试集3：各种数据类型兼容性边界
...
(check (list-tail '(#t #f #\a "hello" 'symbol) 2)
  =>
  '(#\a "hello" 'symbol)
) ;check
(check (list-tail '(#(1 2) #u(255) (a b) car) 1)
  =>
  '(#u(255) (a b) car)
) ;check
(check (list-tail '(() (()) ((()))) 1) => '((()) ((()))))
...
(check (list-tail '(#\中 #\文 #\🚀 #\测) 1)
  =>
  '(#\文 #\🚀 #\测)
) ;check
(check (list-tail '(#\newline #\tab #\space #\return) 2)
  =>
  '(#\space #\return)
) ;check
;; 边界测试集5：点对结构和非正规列表边界
...
(check (list-tail (append '(a b) '(c d) '(e f)) 4)
  =>
  '(e f)
) ;check
(check (list-tail (reverse '(e d c b a)) 2) => '(c d e))
(check (list-tail (cons 'head (cons 'mid (cons 'tail '()))) 1)
  =>
  '(mid tail)
) ;check
;; 边界测试集7：过程对象和特殊值边界
...
(check (list-tail '(define lambda if cond else) 3)
  =>
  '(cond else)
) ;check
;; 边界测试集8：内存结构共享验证边界
