...
;; 复数类型边界测试
(check (memv 1.0+2.0i '(0.0+1.0i 1.0+2.0i 3.0+4.0i))
  =>
  '(1.0+2.0i 3.0+4.0i)
) ;check
(check (memv 0.0 '(1.0+1.0i 2.0+2.0i)) => #f)
;; 有理数边界测试
(check (memv 1/2 '(1/3 1/2 2/3)) => '(1/2 2/3))
(check (memv 3/4 '(1/2 1/4)) => #f)
;; 极端边界值测试
(check (memv 2147483647 '(2147483646 2147483647 2147483648))
  =>
  '(2147483647 2147483648)
) ;check
(check (memv -2147483648 '(-2147483649 -2147483648 -2147483647))
  =>
...
(check (memv 9999.999 '(9999.998 9999.999 10000.0))
  =>
  '(9999.999 10000.0)
) ;check
;; 混合类型列表测试
...
(check (memv 2.5 (cons 1.0 (cons 2.5 (cons 3.0 '()))))
  =>
  '(2.5 3.0)
) ;check
;; 向量与字符混合测试
