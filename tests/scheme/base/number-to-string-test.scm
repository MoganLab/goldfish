...
(check (number->string 2147483647)
  =>
  "2147483647"
) ;check
(check (number->string -2147483648)
  =>
  "-2147483648"
) ;check
;; 基本进制转换测试
(check (number->string 123 2)
  =>
  "1111011"
) ;check
...
(check (number->string 3/4 2)
  =>
  "11/100"
) ;check
(check (number->string 1/3 16) => "1/3")
;; 浮点数转换测试
(check (number->string 123.456)
  =>
  "123.456"
) ;check
(check (number->string 0.0) => "0.0")
(check (number->string -0.123)
  =>
  "-0.123"
) ;check
(check (number->string 0.3) => "0.3")
(check (number->string -0.3) => "-0.3")
(check (number->string 1.23e+10)
  =>
  "1.23e+10"
) ;check
(check (number->string 0.3) => "0.3")
(check (number->string -0.3) => "-0.3")
(check (number->string 0.00123)
  =>
  "0.00123"
) ;check
(check (number->string -0.00123)
...
;; 复数转换测试
(check (number->string 1.0+2.0i)
  =>
  "1.0+2.0i"
) ;check
(check (number->string 0.0+2.0i)
  =>
  "0.0+2.0i"
) ;check
(check (number->string -3.0+4.0i)
  =>
  "-3.0+4.0i"
) ;check
(check (number->string 1.5-2.5i)
  =>
  "1.5-2.5i"
) ;check
(check (number->string 0.0+1.0i)
  =>
  "0.0+1.0i"
) ;check
(check (number->string 0.0) => "0.0")
...
(check (number->string -128 16)
  =>
  "-80"
) ;check
(check (number->string 1023 2)
  =>
  "1111111111"
) ;check
;; 错误处理测试
...
(check-catch 'out-of-range
  (number->string 123 37)
) ;check-catch
(check-catch 'wrong-type-arg
  (number->string 123 3.5)
) ;check-catch
(check-catch 'wrong-number-of-args
  (number->string)
) ;check-catch
(check-catch 'wrong-number-of-args
  (number->string 123 2 3)
) ;check-catch
(check-report)
