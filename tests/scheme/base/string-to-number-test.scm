...
(check (= (string->number "-0.3") (string->number "-3e-1")) => #t)
(check (string->number "1.23e10") => 1.23e+10)
...
;; 复数解析测试
(check (string->number "1+2i") => 1.0+2.0i)
(check (string->number "0+2i") => 0.0+2.0i)
(check (string->number "-3+4i") => -3.0+4.0i)
(check (string->number "3.14-2.71i") => 3.14-2.71i)
(check (string->number "0+1i") => 0.0+1.0i)
...
(check-catch 'out-of-range (string->number "123" -1))
;; 错误参数测试
(check-catch 'type-error (string->number 123))
(check-catch 'type-error (string->number 'symbol))
(check-catch 'type-error (string->number #t))
(check-catch 'type-error (string->number "123" 'not-a-number))
(check-catch 'type-error (string->number "123" 3.5))
