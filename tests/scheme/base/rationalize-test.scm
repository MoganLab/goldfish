...
(check (rationalize 3.14159265359 0.01)
  =>
  22/7
) ;check
(check (rationalize 3.14159265359 0.001)
  =>
  201/64
) ;check
...
(check (rationalize -0.5 0.1) => -1/2)
(check (rationalize 2.71828 0.0001)
  =>
  193/71
) ;check
(check (rationalize 1.4142 0.001)
  =>
  41/29
) ;check
(check (rationalize 2/3 0.05) => 2/3)
(check-catch 'wrong-type-arg
  (rationalize "hello" 0.1)
) ;check-catch
