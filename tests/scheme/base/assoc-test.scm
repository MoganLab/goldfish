...
;; 边界4：列表键精确匹配边界
(check (assoc '(1 2) '(((1 2) . "list-key-1") ((3 4) . "list-key-2")))
  =>
  '((1 2) . "list-key-1")
) ;check
(check (assoc '(list 1 2) '(((list 1 2) . quoted) ((1 2) . unquoted)))
  =>
  '((list 1 2) . quoted)
) ;check
;; 边界5：嵌套结构作为键边界
...
(check (assoc 'nested '((("key" . val) ((key) . "the key")) (missing . other)))
  =>
  #f
) ;check
;; 边界6：重复键处理边界
(check (assoc "test" '(("test" . first) ("test" . second) ("test" . third)))
  =>
  '("test" . first)
) ;check
(check (assoc '(a b) '(((a b) . one) ((a b) . two) ((c d) . three)))
  =>
  '((a b) . one)
) ;check
;; 边界7：数值等价性边界测试
...
;; 边界8：复杂复合对象作为键边界
(check (assoc '((a b) c) '(("((a b) c)" . string)
                           (((a b) c) . list)
                           ((()) . empty)))
  =>
  '(((a b) c) . list)
) ;check
(check (assoc #(1 2 3) '((#(1 2 3) . vector-value) (#(3 4 5) . another)))
  =>
  '(#(1 2 3) . vector-value)
) ;check
;; 字符串详细边界测试
(check (assoc "" '(("" . empty-string))) => '("" . empty-string))
(check (assoc "特殊字符" '(("特殊字符" . "unicode string")))
  =>
  '("特殊字符" . "unicode string")
) ;check
;; 补充assoc完整边界测试
...
;; 边界：Unicode字符串精确匹配
(check (assoc "中文" '(("中文" . "中文值") ("english" . "英文值")))
  =>
  '("中文" . "中文值")
) ;check
...
;; 边界：空字符串和特殊字符
(check (assoc "" '(("" . empty-string))) => '("" . empty-string))
(check (assoc "TeSt" '(("test" . lowercase)
                       ("TeSt" . mixed)
                       ("TEST" . uppercase)))
  =>
  '("TeSt" . mixed)
...
;; 边界11：混合数据类型的键测试
(check (assoc 3.14159 '((1 . "one") (3.14159 . "pi") (2.71828 . "e")))
  =>
  '(3.14159 . "pi")
) ;check
(check (assoc #f '((#t . "true") (#f . "false") (maybe . "unknown")))
  =>
  '(#f . "false")
) ;check
...
(check (assoc 'symbol '((symbol . #t) (number . 42) (string . "test")))
  =>
  '(symbol . #t)
) ;check
;; 边界15：存储过程和复杂对象的值测试
...
;; 边界16：重复前缀匹配测试
(check (assoc "prefix" '(("prefix-match" . 1)
                         ("prefix" . 2)
                         ("prefix-long" . 3)))
  =>
  '("prefix" . 2)
) ;check
(check (assoc 'partial '((partial . 1) (partial-match . 2) (partially . 3)))
  =>
  '(partial . 1)
) ;check
(check-report)
