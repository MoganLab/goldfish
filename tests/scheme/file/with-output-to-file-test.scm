...
(with-output-to-file test-file (lambda () (display "output to file")))
;; 验证文件内容
(check (call-with-input-file test-file (lambda (port) (read-string 100 port)))
  =>
  "output to file"
) ;check
;; 测试覆盖写入
(with-output-to-file test-file (lambda () (display "new output")))
(check (call-with-input-file test-file (lambda (port) (read-string 100 port)))
  =>
  "new output"
) ;check
