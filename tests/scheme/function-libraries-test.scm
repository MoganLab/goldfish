(import (liii check) (liii os) (liii path) (liii string) (goldfish) (liii project))
...
(define (cleanup-function-libraries-fixture base-root)
  (let ((load-root (path-join base-root "goldfish"))
    ...
    (if (path-dir? hidden-root) (path-rmdir hidden-root) #f)
...
(let* ((base-root (path-join (path-temp-dir)
  ...
  (dynamic-wind (lambda () (set! *load-path* (list (path->string load-root))))
    (lambda ()
      (check (function-libraries "unique-func") => '((liii alpha)))
      (check (function-libraries "shared-func") => '((custom beta)
                                                     (liii alpha)))
      (check (function-libraries "fold") => '((srfi 1)))
      (check (function-libraries "renamed-func") => '((custom beta)))
      (check (function-libraries "invisible-func") => '())
      (check (function-libraries "missing-func") => '())
      (check (function-libraries "duplicate-func") => '((liii alpha)))
      (check (function-libraries 1) => '())
    ) ;lambda
    (lambda ()
