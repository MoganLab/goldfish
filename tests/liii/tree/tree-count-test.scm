...
;; 基本功能测试
(check (tree-count 'a '(a b a c a d))
  =>
  3
) ;check
...
(check (tree-count 'b '((a b) (c (b d))))
  =>
  2
) ;check

;; max-count 参数测试
(check (tree-count 'a '(a b a c a d a e) 3)
  =>
  3
) ;check
(check (tree-count 'a '(a b a) 5) => 2)
(check (tree-count 'a '(a b a c) 1)
  =>
  1
) ;check
...
;; 复杂嵌套测试
(check (tree-count 1 '(1 (2 1) (3 (4 1))))
  =>
  3
) ;check
;; 空列表测试：'(() a (b ())) 中有 4 个空列表：
