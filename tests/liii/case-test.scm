...
;; 基本符号匹配
(check (case* 'yes ((yes no) 'boolean) (else 'unknown)) => 'boolean)


(check (case* 'no ((yes no) 'boolean) (else 'unknown)) => 'boolean)


(check (case* 'maybe ((yes no) 'boolean) (else 'unknown)) => 'unknown)


;; 数字匹配
(check (case* 42 ((1 2 3) 'small) ((42 100) 'big) (else 'other)) => 'big)


;; 字符串匹配
(check (case* "hello" (("hi" "hello") 'greeting) (else 'other)) => 'greeting)


;; 混合类型匹配
(check (case* 3.14 ((1 2 3) 'integer) ((3.14 2.71) 'float) (else 'other))
  =>
  'float
) ;check
...
(check (case* '(1 2 3) (((1 2 3)) 'matched) (else 'not-matched)) => 'matched)


;; 不匹配的情况
(check (case* '(1 2 3) (((1 2)) 'two) (((1 2 3)) 'three) (else 'other))
  =>
  'three
) ;check
...
(check (case* '(1 2) (((#<x:> #<y:>)) (+ #<x> #<y>)) (else 0)) => 3)
...
(check (case* 42 ((#<integer?>) 'integer) ((#<string?>) 'string) (else 'other))
  =>
  'integer
) ;check


(check (case* "hello" ((#<integer?>) 'integer) ((#<string?>) 'string) (else 'other))
  =>
  'string
) ;check
...
(check (case* 42 ((#<x:integer?>) (* #<x> 2)) (else 0)) => 84)


;; 自定义谓词函数
(check (case* 10 ((#<even?>) 'even) ((#<odd?>) 'odd) (else 'unknown)) => 'even)


(check (case* 7 ((#<even?>) 'even) ((#<odd?>) 'odd) (else 'unknown)) => 'odd)
...
;; 基本向量匹配
(check (case* #(1 2 3) ((#(1 2 3)) 'matched) (else 'no)) => 'matched)


;; 向量中的标签捕获
(check (case* #(10 20 30) ((#(#<x:> 20 #<y:>)) (list #<x> #<y>)) (else '()))
  =>
  '(10 30)
) ;check
...
;; else 作为默认分支
(check (case* 'unknown-value ((a b c) 'abc) ((x y z) 'xyz) (else 'default))
  =>
  'default
) ;check
...
;; 空列表匹配
(check (case* '() ((()) 'empty) (else 'not-empty)) => 'empty)


;; 单元素列表
(check (case* '(only) ((#<x:>) '#<x>) (else 'none)) => '(only))


;; 单元素向量
(check (case* #(42) ((#(42)) 'forty-two) (else 'none)) => 'forty-two)
