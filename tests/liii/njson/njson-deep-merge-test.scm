...
(let-njson ((base (string->njson deep-merge-base-json)
  ...
  (check (njson-ref merged "meta" "nested" "a")
    =>
    1
  ) ;check
  (check (njson-ref merged "meta" "nested" "b")
    =>
    2
  ) ;check
  ...
  (check (njson-ref merged "override")
    =>
    0
  ) ;check
  (check-catch 'key-error
    (njson-ref base "meta" "y")
  ) ;check-catch
  (check-catch 'key-error
    (njson-ref base "meta" "nested" "b")
  ) ;check-catch
  (check (njson-ref base "override" "k")
    =>
    1
  ) ;check
) ;let-njson
...
(let-njson ((base (string->njson "{\"meta\":{\"x\":1,\"nested\":{\"k\":1}}}"
  ...
  (check (njson->string (njson-deep-merge base base)
         ) ;njson->string
    =>
    (njson->string base)
  ) ;check
