...
(check (codepoint->utf8 127) => #u8(127))
(check (codepoint->utf8 65) => #u8(65))


;; 2 字节编码 (U+0080 到 U+07FF)
(check (codepoint->utf8 164) => #u8(194 164))
(check (codepoint->utf8 128) => #u8(194 128))
(check (codepoint->utf8 2047) => #u8(223 191))


;; 3 字节编码 (U+0800 到 U+FFFF)
(check (codepoint->utf8 20013) => #u8(228 184 173))
(check (codepoint->utf8 2048) => #u8(224 160 128))
(check (codepoint->utf8 65535) => #u8(239 191 191))


;; 4 字节编码 (U+10000 到 U+10FFFF)
(check (codepoint->utf8 128077) => #u8(240 159 145 141))
(check (codepoint->utf8 65536) => #u8(240 144 128 128))
(check (codepoint->utf8 1114111) => #u8(244 143 191 191))


;; 常见字符测试
(check (codepoint->utf8 128640) => #u8(240 159 154 128))
(check (codepoint->utf8 127881) => #u8(240 159 142 137))
...
(check-catch 'value-error (codepoint->utf8 1114112))
