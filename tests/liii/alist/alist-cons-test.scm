...
(check (alist-cons 'c 3 (alist-cons 'b 2 (alist-cons 'a 1 '())))
  =>
  '((c . 3) (b . 2) (a . 1))
) ;check
...
(check (alist-cons 'age 30 '((name . "Alice") (age . 25)))
  =>
  '((age . 30) (name . "Alice") (age . 25))
) ;check


(check (alist-cons 'person '((name . "Alice") (age . 25)) '())
  =>
  '((person (name . "Alice") (age . 25)))
) ;check
(check (alist-cons 'name (list 'first "Alice") '((id . 1)))
  =>
...
(check (alist-cons 'string-value "Hello World" '())
  =>
  '((string-value . "Hello World"))
) ;check
(check (alist-cons 'list-value '(1 2 3 4 5) '()) => '((list-value 1 2 3 4 5)))
(check (alist-cons 'mixed-value '((a 1) (b 2) "string") '())
  =>
...
(let ((result (alist-cons 'step3
                "final"
                (alist-cons 'step2 "process" (alist-cons 'step1 "start" '()))
              ) ;alist-cons
      ) ;result
     ) ;
...
(check (assq 'name (alist-cons 'name "Bob" '((name . "Alice") (age . 25))))
  =>
  '(name . "Bob")
) ;check
(check (assq 'age (alist-cons 'name "Bob" '((age . 30)))) => '(age . 30))
(check (assq 'new-key (alist-cons 'new-key "value" '((existing . "data"))))
  =>
  '(new-key . "value")
) ;check
