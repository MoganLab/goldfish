...
(let ((fv (flexvector 10 20 30)) (res '()))
  (flexvector-for-each (lambda (x) (set! res (cons x res)))
    fv
  ) ;flexvector-for-each
  (check res => '(30 20 10))
) ;let
...
;; 空向量（不执行）
(let ((count 0))
  (flexvector-for-each (lambda (x) (set! count (+ count 1)))
    (flexvector)
  ) ;flexvector-for-each
  (check count => 0)
) ;let
...
;; 单元素
(let ((fv (flexvector 'only)) (res #f))
  (flexvector-for-each (lambda (x) (set! res x))
    fv
  ) ;flexvector-for-each
  (check res => 'only)
) ;let
...
(let ((fv1 (flexvector 1 2 3))
  ...
  (flexvector-for-each (lambda (x y)
                         (set! sums (cons (+ x y) sums))
                       ) ;lambda
    fv1
    fv2
  ) ;flexvector-for-each
...
(let ((fv1 (flexvector 1 2))
  ...
  (flexvector-for-each (lambda (x y z)
                         (set! res (cons (+ x y z) res))
                       ) ;lambda
    fv1
    fv2
    fv3
