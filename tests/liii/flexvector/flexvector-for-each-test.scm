...
;; 多向量遍历（同时遍历多个向量）
(let ((fv1 (flexvector 1 2 3)) (fv2 (flexvector 10 20 30)) (sums '()))
  (flexvector-for-each (lambda (x y) (set! sums (cons (+ x y) sums))) fv1 fv2)
  (check sums => '(33 22 11))
) ;let


;; 多向量长度不同取最短
(let ((fv1 (flexvector 1 2 3 4)) (fv2 (flexvector 10 20)) (count 0))
  (flexvector-for-each (lambda (x y) (set! count (+ count 1))) fv1 fv2)
  (check count => 2)
) ;let
