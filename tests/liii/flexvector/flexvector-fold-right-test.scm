...
(let ((fv (flexvector 1 2 3)))
  ...
  ;; 右折叠：1 - (2 - (3 - 0)) = 2
  (check (flexvector-fold-right - 0 fv)
    =>
    2
  ) ;check
) ;let
...
(let ((fv1 (flexvector 1 2 3))
  ...
  (check (flexvector-fold-right (lambda (x y acc) (+ x y acc))
           0
           fv1
           fv2
         ) ;flexvector-fold-right
...
(let ((fv (flexvector #\h #\e #\l #\l #\o)))
  (check (flexvector-fold-right (lambda (ch acc)
                                  (string-append (string ch) acc)
                                ) ;lambda
           ""
           fv
         ) ;flexvector-fold-right
