...
(let ((fv (flexvector 10 20 30)))
  (check (flexvector-fold-right cons '() fv) => '(10 20 30))
) ;let
...
(let ((fv (flexvector 1 2 3)))
  ;; 左折叠：倒序收集
  (check (flexvector-fold (lambda (acc x) (cons x acc)) '() fv) => '(3 2 1))
  ;; 右折叠：正序收集 (参数顺序为 element acc)
  (check (flexvector-fold-right cons '() fv) => '(1 2 3))
) ;let
...
;; 单元素
(let ((fv (flexvector 'a)))
  (check (flexvector-fold-right cons '() fv) => '(a))
) ;let


;; 多向量折叠 (参数顺序为 element1 element2 ... acc)
(let ((fv1 (flexvector 1 2 3)) (fv2 (flexvector 10 20 30)))
  (check (flexvector-fold-right (lambda (x y acc) (+ x y acc)) 0 fv1 fv2) => 66)
) ;let
...
(let ((fv (flexvector #\h #\e #\l #\l #\o)))
  (check (flexvector-fold-right (lambda (ch acc) (string-append (string ch) acc)) "" fv)
    =>
    "hello"
  ) ;check
) ;let
