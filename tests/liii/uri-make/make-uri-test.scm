...
;; HTTPS URI
(define u2
  (make-uri "https://example.com/path/to/file"
  ) ;make-uri
) ;define
...
;; 带端口的 URI
(define u3
  (make-uri "http://example.com:8080/api")
) ;define
...
;; 带查询字符串的 URI
(define u4
  (make-uri "https://example.com/search?q=hello&page=1"
  ) ;make-uri
) ;define
...
;; 带 fragment 的 URI
(define u5
  (make-uri "https://example.com/docs#section-1"
  ) ;make-uri
) ;define
...
;; 完整 URI
(define u6
  (make-uri "https://user:pass@host:8443/path?a=1#frag"
  ) ;make-uri
) ;define
...
;; Git SSH 格式
(define u7
  (make-uri "git@github.com:liii/uri.git")
) ;define
