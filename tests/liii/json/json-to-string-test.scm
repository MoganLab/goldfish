...
(check (json->string #(1 2 3))
  =>
  "[1,2,3]"
) ;check
(check (json->string '((a (b . 1) (c . 2))))
  =>
  "{a:{b:1,c:2}}"
) ;check
...
(check (json->string #(1 "a" true null))
  =>
  "[1,\"a\",true,null]"
) ;check
(check (json->string '(("name" . "Alice")))
  =>
  "{\"name\":\"Alice\"}"
) ;check
...
(check (json->string '((name . "Bob")))
  =>
  "{name:\"Bob\"}"
) ;check
(check (json->string '((x . 10) (y . 20)))
  =>
  "{x:10,y:20}"
) ;check
(check (json->string #((("id" . 1)) (("id" . 2)))
       ) ;json->string
  =>
  "[{\"id\":1},{\"id\":2}]"
) ;check
(check (json->string '(("scores" . #(85 90 95)))
       ) ;json->string
  =>
  "{\"scores\":[85,90,95]}"
) ;check
(check (json->string '(("user" ("name" . "Dave") ("tags" . #("admin" "editor"))))
       ) ;json->string
  =>
...
(check (json->string #("He said \"Hello\""))
  =>
  "[\"He said \\\"Hello\\\"\"]"
) ;check
...
(define sample-j
  '((user (id . 1001) (name . "Alice") (active . #t) (email . null) (tags . #("dev" "scheme" "json")) (profile (age . 21) (height . 168.5) (hobbies . #("music" "reading")))) (scores . #(98 87 93)))
) ;define
