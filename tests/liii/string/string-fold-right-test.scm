...
;; 内容处理测试 - 字符连接方向验证
(check (string-fold-right (lambda (c acc) (string-append acc (string c))) "" "abc")
  =>
  "cba"
) ;check

;; 统计分析测试
(check (string-fold-right (lambda (c acc) (if (char=? c #\l) (+ acc 1) acc))
         0
         "hello world"
       ) ;string-fold-right
  =>
...
;; ASCII码累加求和
(check (string-fold-right (lambda (c total) (+ total (char->integer c))) 0 "AB")
  =>
  131
) ;check
...
;; Unicode字符测试
(check (string-fold-right (lambda (c acc) (+ acc 1)) 0 "测试")
  =>
  (string-length "测试")
) ;check
...
(check-catch 'out-of-range
  (string-fold-right (lambda (c acc) (+ acc 1)) 0 "hello" -1)
) ;check-catch
(check-catch 'out-of-range
  (string-fold-right (lambda (c acc) (+ acc 1)) 0 "hello" 0 6)
) ;check-catch
(check-catch 'out-of-range
  (string-fold-right (lambda (c acc) (+ acc 1)) 0 "hello" 3 2)
) ;check-catch
(check-catch 'out-of-range
  (string-fold-right (lambda (c acc) (+ acc 1)) 0 "" 1 2)
) ;check-catch
