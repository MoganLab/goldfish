...
;; 相关实现
;; --------
;; (liii string-cursor) 库中也提供了 string-count 函数，
;; 该版本支持 Unicode 字符级别的操作，并提供 cursor-based API。
;; 参见: gf doc liii/string-cursor "string-count"
...
(check (string-count "hello world" #\l 0 5) => 2)
(check (string-count "hello world" #\l 6 11) => 1)
(check (string-count "hello world" #\l 0 3) => 1)
(check (string-count "hello world" #\l 4 8) => 0)

;; 谓词与范围组合测试
(check (string-count "abc123ABC" char-lower-case? 0 6) => 3)
(check (string-count "abc123ABC" char-upper-case? 3 9) => 3)
(check (string-count "Programming123" char-numeric? 11) => 3)
(check (string-count "123456789" char-numeric? 3 6) => 3)
...
;; 自定义谓词测试
(check (string-count "hello world" (lambda (c) (or (char=? c #\l) (char=? c #\o))))
  =>
  5
) ;check
(check (string-count "test123"
         (lambda (c) (or (char=? c #\t) (char=? c #\s) (char=? c #\e)))
       ) ;string-count
  =>
  4
) ;check
(check (string-count "SPECIAL#chars" (lambda (c) (not (char-alphabetic? c))))
  =>
  1
) ;check
