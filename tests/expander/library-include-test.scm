(import (liii check) (liii string) (goldfish expander))
...
;; kernel 库（goldfish expander）经 load-library! / import 正式接入：
...
(load-library! '(goldfish expander))
(check (not (not (runtime-registered? '(goldfish expander)))) => #t)
